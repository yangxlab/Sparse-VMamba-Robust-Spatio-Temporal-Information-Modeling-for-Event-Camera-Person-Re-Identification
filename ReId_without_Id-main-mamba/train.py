...
def train_model(model_ReId, optimizer, scheduler, device, dataset, num_epochs=50):
    # model_E2VID = model_E2VID.eval()
    start_time = time.time()
    writer = SummaryWriter()
    dataloaders, dataset_sizes_allSample = load_train_data(dataset = dataset)
    print("Data size:", dataset_sizes_allSample)

    best_mAP = 0.0  # 用于跟踪最佳平均精度
    best_model_path_map = 'best_model_map.pth'  # 最优模型保存路径

    best_rank = 0.0  # 用于跟踪最佳平均精度
    best_model_path_rank = 'best_model_rank.pth'  # 最优模型保存路径

    for epoch in range(num_epochs):
        batch_count = 0

        if epoch == 0:
            save_network(model_ReId, best_model_path_map)
            save_network(model_ReId, best_model_path_rank)
        ...
        for phase in ['train']:
            ...
            for data in dataloaders:
                ...
                # input_voxel2 = Variable(input_voxel2.to(device))
                labels = Variable(labels.to(device))

                optimizer.zero_grad()
                
                """AN + ReId model joint training"""
                # [category, category_y,feature, _, AN_voxel] = model_ReId(input_voxel,input_voxel2)
                [category, category_y,feature,_, AN_voxel] = model_ReId(input_voxel)
                # voxel_recons_loss = voxel_img_reconst_loss(input_voxel, AN_voxel)
                _, category_preds = torch.max(category.data, 1)

                if opt.ReId_loss == 'softmax+triplet':
                    loss_reid1 = criterion_softmax(category, labels)
                    loss_reid2 =  criterion_softmax(category_y, labels)
                ...
                writer.add_scalars(f'batch_loss', {
                    'reid_loss1': loss_reid1,
                    'reid_loss2': loss_reid2
                }, batch_count)

                loss = loss_reid1 + loss_reid2
                loss.backward()
                optimizer.step()
                ...
                # 打印输出内容
                print(f'Epoch: {epoch} '
                      f'Batch: {batch_count}/{dataset_sizes_allSample}    '
                    #   f'Voxel Reconstruction Loss: {voxel_recons_loss:.4f}, '
                    #   f'triplet Loss: {loss_triplet.item():.4f}',
                    #   f'Image Reconstruction Loss: {img_recons_loss:.4f}, '
                      f'ReID Loss1: {loss_reid1.item():.4f}',
                      f'ReID Loss2: {loss_reid2.item():.4f}',
            ...
            # 每五个epoch测试一次模型
            if (epoch + 1) % 1 == 0:
                CMC, test_mAP = test_reid(model_ReId,dataset)  # 调用测试函数
                print('Embed-Feature top1:%f top5:%f top10:%f mAP:%f'%(CMC[0], CMC[4], CMC[9], test_mAP))
                if test_mAP > best_mAP:
                    best_mAP = test_mAP
                    save_network(model_ReId, best_model_path_map)  # 保存最佳模型
                    print(f'Best_Epoch_map: {epoch} '+'Embed-Feature top1:%f top5:%f top10:%f mAP:%f'%(CMC[0], CMC[4], CMC[9], test_mAP))
                if CMC[0] > best_rank:
                    best_rank = CMC[0]
                    save_network(model_ReId, best_model_path_rank)  # 保存最佳模型
                    print(f'Best_Epoch_rank: {epoch} '+'Embed-Feature top1:%f top5:%f top10:%f mAP:%f'%(CMC[0], CMC[4], CMC[9], test_mAP))
...
def save_network(network1, save_filename): 
...
def load_train_data(dataset, num_workers=8):

    """Load Train Data"""
    if opt.represent == "voxel":

        if opt.ReId_loss == 'softmax+triplet':
            cls_datasets = voxelDataset(event_dir = dataset, mode='train')
            cls_loader = torch.utils.data.DataLoader(cls_datasets,
                                                     sampler=RandomIdentitySampler(cls_datasets, opt.batchsize, opt.num_instances),
        ...
        print("sixe of len(cls_datasets)",a)
...
def set_optimizer(model):
    ...
    ignored_params +=  list(map(id, model.head2.parameters()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_ids', default='1', type=str,help='gpu_ids: e.g. 0  0,1')
    parser.add_argument('--name', default='AN_Event_ReId', type=str, help='output model name')
    parser.add_argument('--num_ids', default=44, type=int, help='number of identities')
    ...
    """Start Training"""
    model_ = train_model(reid_model, optimizer, None, device, dataset= opt.datasets, num_epochs=opt.epoch)
