...
class EvReId(nn.Module):
    def __init__(self, class_num=22, num_channel=None, AE_block=None):
        ...
        model_path = '/data/dwj/vssm_small_0229_ckpt_epoch_222.pth'
        backbone.load_pretrained(model_path)
        print('Loading pretrained ImageNet model......from {}'.format(model_path))
        self.model = backbone
        # self.mlp = FRFN(dim=768, hidden_dim=768 * 4)
        # self.mlp.apply(weights_init_kaiming)
        #self.block_num = block_num
        # self.classifier = ClassBlock(768, class_num, dropout=False, relu=False, num_bottleneck=768)
    ...
    def forward(self, x):
        ...
        cash_x, cash_y = self.model(x) 
        category = self.head(cash_x)  # 线性分类
        category_y = self.head2(cash_y)  # 线性分类
        # print(cash_x.shape)torch.Size([32, 768, 12, 6])
        # print(cash_x.shape)torch.Size([64, 1024, 8, 4])
        # global_feat = torch.mean(cash_x, dim=(-2, -1))
        # # print(global_feat.shape)torch.Size([32, 768])

        # feature = self.classifier.add_block_1(global_feat)
