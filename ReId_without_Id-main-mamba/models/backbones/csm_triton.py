...
class CrossScanTritonF(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0, grad_enabled=True):
        if one_by_one:
            if in_channel_first:
                B, _, C, H, W = x.shape
            else:
        ...
        #     if x.shape[1] == 1536:
        #         # print("mask!!!!!!!!!!!!!")
        #         x = CrossScanTritonF.drop_random_patches(x)
        ...
        if grad_enabled:  # 只有在训练时 (即梯度启用时) 才会应用掩码
            if y.ndimension() == 4 and y.shape[2] == 1536:
                # print("mask here!!!!!!!!!!!")
                y = CrossScanTritonF.apply_random_mask(y)
    ...
    @staticmethod
    def drop_random_patches(x, drop_rate=0.05):
        B, C, H, W = x.size()
        ...
        # 计算要丢弃的 patches 数量
        num_patches_to_drop = int(total_patches * drop_rate)
        
        # 随机选择要丢弃的 patches 的索引
        all_patch_indices = list(range(total_patches))
        dropped_indices = random.sample(all_patch_indices, num_patches_to_drop)
        ...
        for index in dropped_indices:
            i = index // W  # 计算 patch 的行索引
            j = index % W   # 计算 patch 的列索引
            
            # 设置掩码为 0
            mask[:, :, i, j] = 0

        # 应用掩码
        retained_patches = x * mask  # 将掩码应用于原始张量

        return retained_patches
    
    @staticmethod
    def apply_random_mask(y: torch.Tensor):
        """
        对于每种扫描方式生成随机掩码并应用到 y。
        """
        # 获取输入张量的维度
        B, num_scans, C, H_W = y.shape  # 这里 H_W = H * W
        
        # 为每个扫描方式生成一个掩码 (在训练时应用掩码)
        random_mask = torch.rand(B, num_scans, C, H_W, device=y.device) < 0.95
        
        # 将掩码应用到 y
        return y * random_mask  # 应用掩码，遮住部分 patch
...
def cross_scan_fn(x: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0, force_torch=False):
    ...
    with torch.cuda.device(x.device):
        return CSF.apply(x, in_channel_first, out_channel_first, one_by_one, scans, torch.is_grad_enabled())
