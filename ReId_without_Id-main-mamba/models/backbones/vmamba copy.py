...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # self.crossmamba = CrossVSSBlock(hidden_dim=192, 
        #         drop_path=0.1,
        #         norm_layer=norm_layer,
        #         channel_first=self.channel_first,
        #         ssm_d_state=ssm_d_state,
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print(z.shape)([4, 96, 96, 48])
        # x = x+x1
        # x_ori = self.spatial_attention(x_ori)
        ...
        # x = x + x1
        # x = self.crossmamba(x, x1)
        # print(x.shape)torch.Size([32, 192, 48, 24])
        # z = self.layers[0](z) # 第一层
        
        # x =   x +  z
        # x =  0.7 * x + 0.3 * z
        # x = self.crossmamba(x, z)
        
        
        
        
        # print(x1.shape)torch.Size([32, 192, 12, 6])
        ...
        # print(x.shape)torch.Size([4, 192, 48, 24])
        # x = x + self.spatial_attention(x_spcial.contiguous())
        # x_spcial = x
        ...
        # print(x.shape)torch.Size([4, 384, 24, 12])
        # x = x + self.spatial_attention(x_spcial.contiguous())
        # x_spcial = x
        # # x = self.spatial_attention(x) + x
        ...
        # print(x.shape)torch.Size([4, 768, 12, 6])
        # x_attention = self.spatial_attention(x)
        # x_spcial = x
        # # x = self.spatial_attention(x) + x
        part_feat1 =  self.layers[3](x)
        global_feat = part_feat1 + x
        # global_feat = self.crossmamba(global_feat1 , x_attention)
        # print(x.shape)torch.Size([4, 768, 12, 6])
        # best!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # x = self.spatial_attention(x.contiguous()) + x
