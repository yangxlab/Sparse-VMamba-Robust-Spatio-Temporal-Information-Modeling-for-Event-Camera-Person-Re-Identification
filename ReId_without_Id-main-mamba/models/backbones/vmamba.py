...
class CrossVSSBlock(nn.Module):
    ...
    def _forward(self, input1: torch.Tensor, input2: torch.Tensor):
        ...
        if self.ssm_branch:
            # 对两个模态都进行 SSM 处理
            if self.post_norm:
                x1 = self.op(self.norm(x1))
                x2 = self.op(self.norm(x2))
            else:
                x1 = x1 + self.drop_path(self.op(self.norm(x1)))
                # print(x1.shape)torch.Size([4, 768, 12, 6])
                x2 = x2 + self.drop_path(self.op(self.norm(x2)))
                # print(x2.shape)torch.Size([4, 768, 12, 6])
            
            x1_out = x1
 # 计算相似度（使用点积）
            # 先展平最后两个维度
            x1_flat = x1.view(x1.size(0), x1.size(1), -1)  # [4, 768, 72]
            x2_flat = x2.view(x2.size(0), x2.size(1), -1)  # [4, 768, 72]

            # 计算相似度
            similarity = torch.bmm(x1_flat.transpose(1, 2), x2_flat)  # [4, 72, 72]
            # print(similarity)
            ...
            # 将相似度加权到 x2
            weighted_x2 = torch.bmm(similarity, x2_flat.transpose(1, 2)).transpose(1, 2)  # 加权
            # print(weighted_x2.shape)torch.Size([4, 768, 72])
            x1 = x1 + weighted_x2.view(b,c,h,w)  # 将加权后的 x2 注入到 x1

        if self.mlp_branch:
            if self.post_norm:
                x1 = x1 + self.drop_path(self.norm_mlp(self.mlp(x1)))  # MLP
            else:
                x1 = x1 + self.drop_path(self.mlp(self.norm_mlp(x1)))  # MLP
    ...
    def forward(self, input1: torch.Tensor, input2: torch.Tensor):
        if self.use_checkpoint:
            ...
            return self._forward(input1, input2)


# class SpatialAttention(nn.Module):  ##一阶
...
# class SpatialAttention(nn.Module): ##二阶
#     def __init__(self, kernel_size=7):
#         super(SpatialAttention, self).__init__()
#         self.conv1 = nn.Conv2d(2, 1, kernel_size, padding=kernel_size // 2, bias=False)
#         self.sigmoid = nn.Sigmoid()
...
#     def forward(self, x):
#         # 使用Laplacian算子计算二阶导数
#         # 水平方向的二阶导数: f(x-1) - 2*f(x) + f(x+1)
#         laplacian_x = x[:, :, :, :-2] - 2 * x[:, :, :, 1:-1] + x[:, :, :, 2:]
#         laplacian_x = F.pad(laplacian_x, (1, 1, 0, 0), "reflect")
        
#         # 垂直方向的二阶导数: f(y-1) - 2*f(y) + f(y+1)
#         laplacian_y = x[:, :, :-2, :] - 2 * x[:, :, 1:-1, :] + x[:, :, 2:, :]
#         laplacian_y = F.pad(laplacian_y, (0, 0, 1, 1), "reflect")
        
#         # 拼接两个方向的Laplacian
#         spatial_laplacian = torch.cat((laplacian_x, laplacian_y), dim=1)

#         min_val = torch.min(spatial_laplacian)
#         max_val = torch.max(spatial_laplacian)
#         # 归一化
#         normalized_laplacian = (spatial_laplacian - min_val) / (max_val - min_val + 1e-8)

#         avg_out = torch.mean(normalized_laplacian, dim=1, keepdim=True)
#         max_out, _ = torch.max(normalized_laplacian, dim=1, keepdim=True)
#         attention = torch.cat([avg_out, max_out], dim=1)
#         attention = self.conv1(attention)
#         return x * self.sigmoid(attention)

# class SpatialAttention(nn.Module):   ##sobel
#     def __init__(self, kernel_size=7):
#         super(SpatialAttention, self).__init__()
#         self.conv1 = nn.Conv2d(2, 1, kernel_size, padding=kernel_size // 2, bias=False)
#         self.sigmoid = nn.Sigmoid()
...
#             sobel_x_list.append(gx)
            
#             # 应用Sobel_y
#             gy = F.conv2d(channel, self.sobel_y, padding=1)
#             sobel_y_list.append(gy)
        
#         # 拼接所有通道
#         sobel_x = torch.cat(sobel_x_list, dim=1)
#         sobel_y = torch.cat(sobel_y_list, dim=1)
        
#         # 拼接两个方向的Sobel梯度
#         spatial_sobel = torch.cat((sobel_x, sobel_y), dim=1)

#         min_val = torch.min(spatial_sobel)
#         max_val = torch.max(spatial_sobel)
#         # 归一化
#         normalized_sobel = (spatial_sobel - min_val) / (max_val - min_val + 1e-8)

#         avg_out = torch.mean(normalized_sobel, dim=1, keepdim=True)
#         max_out, _ = torch.max(normalized_sobel, dim=1, keepdim=True)
#         attention = torch.cat([avg_out, max_out], dim=1)
#         attention = self.conv1(attention)
#         return x * self.sigmoid(attention)
...
class SpatialAttention(nn.Module):
    ...
    def canny_like_edge_detection(self, x):
        """类Canny边缘检测 - 稳定版本"""
        # 使用PyTorch内置操作避免cuDNN问题
        ...
        # 2. 计算梯度 - 使用简单差分
        # X方向梯度
        gx = smoothed[:, :, :, 2:] - smoothed[:, :, :, :-2]
        gx = F.pad(gx, (1, 1, 0, 0), mode='replicate')
        
        # Y方向梯度
        gy = smoothed[:, :, 2:, :] - smoothed[:, :, :-2, :]
        gy = F.pad(gy, (0, 0, 1, 1), mode='replicate')
        
        # 3. 梯度幅值
        gradient_magnitude = torch.sqrt(gx**2 + gy**2 + 1e-8)
        ...
        suppressed = torch.where(
            torch.abs(gradient_magnitude - max_pooled) < 1e-6,
            gradient_magnitude,
            torch.zeros_like(gradient_magnitude)
        )
        
        # 5. 双阈值
        # 计算自适应阈值
        flat = suppressed.view(suppressed.size(0), suppressed.size(1), -1)
        sorted_vals, _ = torch.sort(flat, dim=2, descending=True)
        
        # 取前10%作为高阈值，前30%作为低阈值
        high_idx = max(1, int(sorted_vals.size(2) * 0.1))
        low_idx = max(1, int(sorted_vals.size(2) * 0.3))
        
        high_threshold = sorted_vals[:, :, high_idx:high_idx+1].unsqueeze(-1)
        low_threshold = sorted_vals[:, :, low_idx:low_idx+1].unsqueeze(-1)
        ...
        for _ in range(3):  # 迭代3次
            # 膨胀强边缘
            dilated = F.max_pool2d(edges, kernel_size=3, stride=1, padding=1)
            # 连接相邻的弱边缘
            edges = torch.clamp(edges + (dilated * weak_edges), 0, 1)
        
        return edges

    def forward(self, x):
        # Canny边缘检测
        edges = self.canny_like_edge_detection(x)
        
        # 计算注意力
        avg_out = torch.mean(edges, dim=1, keepdim=True)
        max_out, _ = torch.max(edges, dim=1, keepdim=True)
        attention = torch.cat([avg_out, max_out], dim=1)
        attention = self.conv1(attention)
...
class GRUBlock(nn.Module):
    ...
    def forward(self, x):
        # x 的形状为 [batch_size, time_steps, input_size]
        # GRU 输出
        gru_out, _ = self.gru(x)  # gru_out 的形状为 [batch_size, time_steps, hidden_size]
        
        # 取最后一个时间步的输出
        last_time_step = gru_out[:, -1, :]  # 形状为 [batch_size, hidden_size]
        
        # 通过线性层输出
        output = self.fc(last_time_step)  # 形状为 [batch_size, output_size]
...
class LSTMBlock(nn.Module):
    ...
    def forward(self, x):
        # x 的形状为 [batch_size, time_steps, input_size]
        # LSTM 输出
        lstm_out, _ = self.lstm(x)  # lstm_out 的形状为 [batch_size, time_steps, hidden_size]
        
        # 取最后一个时间步的输出
        last_time_step = lstm_out[:, -1, :]  # 形状为 [batch_size, hidden_size]
...
class RNNBlock(nn.Module):
    ...
    def forward(self, x):
        # x 的形状为 [batch_size, time_steps, input_size]
        # RNN 输出
        rnn_out, _ = self.rnn(x)  # rnn_out 的形状为 [batch_size, time_steps, hidden_size]
        
        # 取最后一个时间步的输出
        last_time_step = rnn_out[:, -1, :]  # 形状为 [batch_size, hidden_size]
        
        # 通过线性层输出
        output = last_time_step  # 形状为 [batch_size, output_size]
        
        return output

class PatchRNN(nn.Module):
    def __init__(self, patch_size=4, input_channels=1, hidden_size=96, output_size=96, num_layers=1):
        ...
        # # RNN block for each patch
        # self.rnn_block = RNNBlock(input_size=input_channels * patch_size * patch_size,
        #                            hidden_size=hidden_size,
        #                            output_size=output_size)
        
         # RNN block for each patch
        # self.rnn_block = LSTMBlock(input_size=input_channels * patch_size * patch_size,
        #                            hidden_size=hidden_size,
        #                            output_size=output_size)
        self.rnn_block = GRUBlock(input_size=input_channels * patch_size * patch_size,
                                   hidden_size=hidden_size,
                                   output_size=output_size)

    def forward(self, x):
        # x 的形状为 [batch_size, time_steps, height, width]
        batch_size, time_steps, height, width = x.size()
        
        # 将输入分割成 patches
        patches = x.unfold(2, self.patch_size, self.patch_size).unfold(3, self.patch_size, self.patch_size)
        # 现在 patches 的形状为 [batch_size, time_steps, num_patches_height, num_patches_width, patch_size, patch_size]
        
        patches = patches.contiguous().view(batch_size, time_steps, -1, self.patch_size * self.patch_size)
        # 形状变为 [batch_size, time_steps, num_patches, patch_size * patch_size]

        # 直接将所有 patches 送入 RNN
        # 重新排列维度为 [batch_size * num_patches, time_steps, patch_size * patch_size]
        num_patches = patches.size(2)
        patches = patches.view(batch_size, time_steps, num_patches, self.patch_size * self.patch_size)  # [batch_size, time_steps, num_patches, patch_size * patch_size]
        patches = patches.permute(0, 2, 1, 3).contiguous().view(batch_size * num_patches, time_steps, self.patch_size * self.patch_size)  # [batch_size * num_patches, time_steps, patch_size * patch_size]

        # 对所有 patches 应用 RNN
        output = self.rnn_block(patches)  # [batch_size * num_patches, output_size]

        # 将输出重塑为 [batch_size, num_patches, output_size]
        output = output.view(batch_size, num_patches, self.output_size)

        # 需要调整输出形状
        num_patches_height = height // self.patch_size
        num_patches_width = width // self.patch_size
        
        # 将输出重塑为 [batch_size, output_size, num_patches_height, num_patches_width]
        output = output.permute(0, 2, 1).view(batch_size, self.output_size, num_patches_height, num_patches_width)
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_embed = _make_patch_embed(5, dims[0], patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
        # self.patch_embed2 = _make_patch_embed(3, dims[0], patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
        self.patchembed_event = PatchRNN()
        ...
        # self.norm = norm_layer(self.num_features)  # 归一化层
        ...
        # 定义维度调整层
        if not self.channel_first:
            self.permute = nn.Permute(0, 3, 1, 2)  # 从 (B, H, W, C) 到 (B, C, H, W)
        else:
            self.permute = nn.Identity()  # 不改变维度顺序
        
        # 定义自适应平均池化层
        self.avgpool = nn.AdaptiveAvgPool2d(1)  # 输出为 (B, C, 1, 1)
        
        # 定义展平层
        self.flatten = nn.Flatten(1)  # 展平为 (B, C)
        
        # 定义线性层
        self.forward_count = 0
        # log_file = '/data/dwj/ReId_without_Id-main-mamba/recordv2.csv'
        ...
        #         writer = csv.writer(f)
        #         writer.writerow(['Step', 'Learnable_Param'])
    ...
    @staticmethod
    def _make_patch_embed_v2(in_chans=5, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        stride = patch_size // 2
    ...
    def forward(self, x: torch.Tensor,datasets):
        # print(x.shape)torch.Size([32, 5, 384, 192])
        # print(x.shape)torch.Size([32, 5, 384, 192])
        x_ori = self.patch_embed(x)  #cnn
        # print(x_ori.shape)([4, 96, 96, 48])
        # x1 = self.patch_embed2(x1)
        z = self.patchembed_event(x)  #gru


        x = x_ori + self.learnable_param * z

        x = self.layers[0](x) # 第一层
        x = self.spatial_attention1(x)
        
        
        x = self.layers[1](x)   # 第二层
        
        x = self.layers[2](x)   # 第三层
      
        part_feat1 =  self.layers[3](x)
        global_feat = part_feat1 + x
       
        feature = self.classifier(global_feat)  # 归一化
        x = self.permute(feature)  # 调整维度
        x = self.avgpool(x)  # 自适应平均池化
        x = self.flatten(x)  # 展平
        
        y = self.classifier(part_feat1)
        y = self.permute(y)  # 调整维度
        y = self.avgpool(y)  # 自适应平均池化
        y = self.flatten(y)  # 展平
    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        try:
            ...
            pretrained_dict = _ckpt[key]

            # 过滤掉不匹配的权重
            filtered_pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in current_state_dict and current_state_dict[k].size() == v.size()}
             
            # # # 打印加载的参数
            # loaded_keys = filtered_pretrained_dict.keys()
            # print(f"Loaded parameters: {list(loaded_keys)}")

            # 更新当前模型的状态字典
            current_state_dict.update(filtered_pretrained_dict)
...
def vmamba_small_s2l15(num_classes=1000,
            imgsize=224,channel_first=True):
    return VSSM(
        depths=[2, 2, 15, 2], dims=96, drop_path_rate=0.3, 
        patch_size=4, in_chans=5, num_classes=num_classes, 
        ssm_d_state=1, ssm_ratio=2.0, ssm_dt_rank="auto", ssm_act_layer="silu",
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0, 
        ssm_init="v0", forward_type="v05_noz", 
        mlp_ratio=4.0, mlp_act_layer="gelu", mlp_drop_rate=0.0, gmlp=False,
        patch_norm=True, norm_layer=("ln2d" if channel_first else "ln"), 
        downsample_version="v3", patchembed_version="v2", 
        use_checkpoint=False, posembed=False, imgsize=imgsize, 
    )


def vmamba_base_s2l15(num_classes=1000,
            imgsize=224,channel_first=True):
    return VSSM(
...
# =====================================================
def vmamba_tiny_s1l8(num_classes=1000,
            imgsize=224,channel_first=True):
    return VSSM(
