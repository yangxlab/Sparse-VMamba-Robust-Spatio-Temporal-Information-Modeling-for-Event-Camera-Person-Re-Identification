...
# 该函数按照给定的事件数量（N）进行分段。每个体素的大小是固定的，具体为 N 个事件。
...
#根据给定的集成时间（integ_time）进行分段。每个体素的时间长度是固定的，具体为 integ_time 毫秒。
    ts, x, y, pol = load_events(complete_data_path)
    id_, cam = id_cam(complete_data_path)
    save_dir = out_dir + '/' + complete_data_path + '/'
...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default='out_dir_v2/', help='event stream directory')
    ...
    for i in range(43,60):
        i_d = "{0:03}".format(i + 1)
        data_path = os.path.join(params.input_dir, str(i_d))
        ...
        if os.path.exists(data_path):
            ...
            # 只处理存在的相机文件夹
            for cam in sub_path:
                complete_data_path = os.path.join(data_path, cam)
                events_file_path = os.path.join(complete_data_path, "events.txt")  # 构建完整的文件路径

                if os.path.exists(events_file_path):  # 检查文件是否存在
