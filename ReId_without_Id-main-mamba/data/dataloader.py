...
class voxelDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if events.size == 0 or events.shape[0] == 0:
            ...
            # voxel_grid2 = events_to_voxel_grid(events, num_bins=3, width=max(events[:, 1])+1, height=max(events[:, 2])+1)

            # # Numpy 转换为 Tensor
            # voxel_grid2 = torch.tensor(voxel_grid2, dtype=torch.float32)

            # # 归一化体素网格
            # voxel_grid2 = normalize_voxel(voxel_grid2)

            # # 调整尺寸
            # voxel_grid2 = F.interpolate(voxel_grid2.unsqueeze(0), size=(384, 192), mode='nearest').squeeze(0)
